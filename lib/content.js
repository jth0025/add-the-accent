...
export function getAllEntries(section) {
  ...
  const entries = files.map((filename) => {
    ...
    return {
      ...
      // Optional pre-baked HTML variant of the title (e.g. with an <em>
      // in it), used by display components instead of the plain title
      // when present. Keep `title` itself plain — it still backs
      // metadata, alt text, and anywhere else HTML would be wrong.
...
// "Selected Work" — the site's portfolio. Rather than a separate content
// type, it curates from content/journal: a hand-picked set of entries
// that best demonstrate range and voice for anyone landing here to
// evaluate the writing (a hiring manager, a collaborator, a reader).
// Edit this list — order included — to change what's featured. Slugs
// with no matching journal entry are silently skipped.
...
export function getSelectedWork() {
  ...
  const bySlug = Object.fromEntries(journal.map((e) => [e.slug, e]));
  return SELECTED_WORK_SLUGS.map((slug) => bySlug[slug]).filter(Boolean);
}
