/**
 * Promo for the Apple Music radio station — a glass box, sized to its
 * own content rather than matched to the voice clip beside it, so the
 * art and "Listen now!" label both have room to actually read. On
 * narrow screens the box holds just the banner art (full-bleed
 * overlay) with a plain "Listen now!" line below it; from md up,
 * "Listen now!" moves inside the box, upright, beside the art — the
 * box becomes a two-part row (art filling the remaining space, label
 * at a fixed width) instead of a single full-bleed image.
 */
export default function RadioBanner() {
  return (
    ...
    >
      <div className="corner-box on-dark flex h-32 w-full items-stretch overflow-hidden rounded-lg border border-white/25 bg-white/10 sm:h-36 md:h-44">
        <div className="relative flex-1">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src="/promo-banner.png"
            alt="Green Maize’s Station — Apple Music radio"
            className="absolute inset-0 h-full w-full object-contain transition-transform duration-500 group-hover:scale-[1.05]"
          />
        </div>
